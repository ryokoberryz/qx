...
var RecordAddurl=$XidN.read("RecordAddurlname1");
var RecordAddhd=$XidN.read("RecordAddhdname1");
var RecordAddbd=$XidN.read("RecordAddbdname1");
  const Record={
      url:RecordAddurl,
...
var RecordListurl=$XidN.read("RecordListurlname1");
var RecordAddhd=$XidN.read("RecordAddhdname1");
var RecordListbd=$XidN.read("RecordListbdname1");
  const RecordList={
      url:RecordListurl,
      headers:JSON.parse(RecordAddhd),
      body:RecordListbd,
...
var objtemp=obj.detail.signRecordList;
 result2+=`[连续签到✨]${obj.detail.ContinueDay+"天"+",账户"+obj.detail.CheckinBonusSum+"积分"}\n`;
...
if(urlval.indexOf("api/user/User/CheckinV2")>=0)
{
var so= $XidN.write(md_bd,"RecordAddbdname1");
 var ao= $XidN.write(urlval,"RecordAddurlname1");
var bo= $XidN.write(JSON.stringify(md_hd),"RecordAddhdname1");

if (ao==true&&bo==true&&so==true) 
 log(dd,"[获取签到数据]","✅成功");}

else
if(urlval.indexOf("api/user/User/GetCheckinDetail")>=0)
{
var so= $XidN.write(urlval,"RecordListurlname1");
 var ao= $XidN.write(md_bd,"RecordListbdname1");
